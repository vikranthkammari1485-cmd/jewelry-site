...
const CATEGORIES = ['All', 'Jewelry', 'Fashion', 'Gadgets', 'Gifts', 'Interiors']
...
function getPlatform(url) {
  ...
  if (u.includes('pepperfry')) return { label: 'Pepperfry', color: '#F37320', icon: '🛋️' }
  if (u.includes('urbanladder')) return { label: 'Urban Ladder', color: '#4CAF50', icon: '🪴' }
  ...
  return (
    <article className="card">
      <div className="card-image-wrap">
        ...
        {isAdmin && (
          <button className="card-delete" onClick={() => onDelete(product.id)}>✕</button>
        )}
      </div>
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="field">
            ...
            <input type="email" className="field-input" placeholder="Enter admin email" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="field">
            <label className="field-label">Password</label>
            <input type="password" className="field-input" placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>
    ...
    <>
      <header className="header">
        <div className="header-inner">
          ...
          <p className="header-tagline">Curated finds • Jewelry · Fashion · Gadgets · Gifts · Interiors</p>
          <div className="admin-corner">
            {isAdmin ? (
              <button className="admin-pill admin-pill--active" onClick={handleLogout}>🔓 Logout</button>
            ) : (
              <button className="admin-pill" onClick={() => setShowLogin(true)}>🔒 Admin</button>
            )}
          </div>
        </div>
      </header>
      ...
      <div className="controls">
        <div className="controls-inner">
          ...
          {isAdmin && (
            <button className="btn-add" onClick={() => setShowAdd(true)}>+ Add Product</button>
          )}
        </div>
      </div>
      ...
      <main className="main">
        {loading ? (
          ...
          <div className="empty">
            <p>{products.length === 0 ? '✨ Products coming soon' : 'No matching products'}</p>
          </div>
        ) : (
          <div className="grid">
            {filtered.map((p) => (
      ...
      <footer className="footer">
        <p>Made with ♥ for PinnedPicks · {new Date().getFullYear()}</p>
      </footer>

      {showLogin && <LoginModal onClose={() => setShowLogin(false)} />}
      {showAdd && isAdmin && <AddProductModal onAdd={handleAdd} onClose={() => setShowAdd(false)} />}
    </>
  )
}
